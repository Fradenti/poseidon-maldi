sparseBC.choosekr_patched <- function (x, k, r, lambda, percent = .1, trace = FALSE)
{
  numberoftimes <- 1 / percent
  if (!isTRUE(all.equal(numberoftimes, round(numberoftimes))))
  ...
  IndicatorMatrix <- 1 * (results.mean[1:(length(k) - 1),
                                       1:(length(r) - 1)] <= results.mean[2:length(k), 2:length(r)] +
                            results.se[2:length(k), 2:length(r)])
