...
# rbind method ------------------------------------------------------------
lip      <- as.matrix(readRDS("../poseRepos/RenalTissue2/Data_OCT24/4k/FinalMatrix_Lipids.RDS"))
gly      <- as.matrix(readRDS("../poseRepos/RenalTissue2/Data_OCT24/4k/FinalMatrix_Glycans.RDS"))
pep      <- as.matrix(readRDS("../poseRepos/RenalTissue2/Data_OCT24/4k/FinalMatrix_Peptides.RDS"))
rownames(lip) = colnames(lip) = NULL
rownames(gly) = colnames(gly) = NULL
rownames(pep) = colnames(pep) = NULL
...
for(i in 1:nsim){
  ...
  saveRDS(MOD,paste0("CRCC_Analysis/Output/RDS/All_runs_Stacked_Pose/Pose_4kStacked_fisan_K30L40_epsilon1e-8_run#",i,".RDS"))
  rm(MOD)
  gc()
}
...
for(i in 1:nsim){
  x <- readRDS(paste0("CRCC_Analysis/Output/RDS/All_runs_Stacked_Pose/Pose_4kStacked_fisan_K30L40_epsilon1e-8_run#",i,".RDS"))
  ELBOS[[i]] <- x$Elbo_val
  cat(i)
}
...
mod1 <- readRDS(paste0("CRCC_Analysis/Output/RDS/All_runs_Stacked_Pose/Pose_4kStacked_fisan_K30L40_epsilon1e-8_run#",ind.max,".RDS"))
saveRDS(mod1,"CRCC_Analysis/Output/RDS/All_runs_Stacked_Pose/Best_Pose_4kStacked.RDS")
