...
/*
// [[Rcpp::export]]
double est_itemp_PL(arma::mat RHO,
                    arma::field<arma::uvec> NN_inds,
                    arma::vec itemp_grid){

  int n_itemp = itemp_grid.n_rows;
  int J = RHO.n_rows;

  arma::mat DEN(J,n_itemp);
  arma::mat lNUM(J,n_itemp);
  double current = 0.0;

  for(int j=0; j<J; j++){
    arma::uvec inds = NN_inds[j];
    current = arma::accu(RHO.row(j) * trans(RHO.rows(inds)));
    arma::rowvec curr = arma::sum(RHO.rows(inds),0);
    for(int b = 0; b < n_itemp; b++){
      DEN(j,b) = arma::accu(exp(itemp_grid[b] * curr));
      lNUM(j,b) = (itemp_grid[b]*current);
    }
  }

  arma::rowvec X = arma::sum(lNUM-log(DEN),0) ;
  arma::mat res =  exp(X - row_LogSumExp_cpp(X)) * itemp_grid ;
  //Rcpp::Rcout<<  exp(X - row_LogSumExp_cpp(X));
  return( res[0] );
